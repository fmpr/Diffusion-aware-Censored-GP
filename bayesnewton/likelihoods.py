...
class Likelihood(objax.Module):
    ...
    def evaluate_likelihood(self, y, f, extra):
        return np.exp(self.evaluate_log_likelihood(y, f, extra))
    ...
    def log_density(self, y, mean, cov, extra, cubature=None):
        """
        """
        return log_density_cubature(self, y, mean, cov, extra, cubature)
...
class MultiLatentLikelihood(Likelihood):
    ...
    def log_density(self, y, mean, cov, extra, cubature=None):
        ...
        weighted_likelihood_eval = w * vmap(self.evaluate_likelihood, in_axes=(None, 1, None))(y, sigma_points, extra)
    ...
    def expected_log_likelihood(self, y, post_mean, post_cov, extra, cubature=None):
        """
        ...
        #print(w.shape, y.shape, sigma_points.shape, extra.shape) # (400,) (2, 1) (2, 400) (2, 1)
        #print(vmap(self.evaluate_log_likelihood, in_axes=(None, 1, None))(y, sigma_points, extra).shape) # (400, 2, 2)
        weighted_log_likelihood_eval = w * vmap(self.evaluate_log_likelihood, in_axes=(None, 1, None))(y, sigma_points, extra)
    ...
    def expected_log_likelihood_dm(self, y, post_mean, post_cov, extra, cubature=None):
        """
        """
        dE_dm = grad(self.expected_log_likelihood, argnums=1)(y, post_mean, post_cov, extra, cubature)
        return dE_dm

    def expected_log_likelihood_dm2(self, y, post_mean, post_cov, extra, cubature=None):
        """
        """
        d2E_dm2 = jacrev(self.expected_log_likelihood_dm, argnums=1)(y, post_mean, post_cov, extra, cubature)
        return np.squeeze(d2E_dm2)

    def variational_expectation(self, y, post_mean, post_cov, extra, cubature=None):
        """
        Compute expected log likelihood via cubature:
        E[log p(yₙ|fₙ)] = ∫ log p(yₙ|fₙ) 𝓝(fₙ|mₙ,vₙ) dfₙ
        """
        E = self.expected_log_likelihood(y, post_mean, post_cov, extra, cubature)
        dE_dm = self.expected_log_likelihood_dm(y, post_mean, post_cov, extra, cubature)
        d2E_dm2 = self.expected_log_likelihood_dm2(y, post_mean, post_cov, extra, cubature)
        return E, dE_dm, d2E_dm2

...
class BaseModel(objax.Module):
    ...
    def negative_log_predictive_density(self, X, Y, R=None, extra=None, cubature=None):
        predict_mean, predict_var = self.predict(X, R) 
        #print(Y.shape, predict_mean.shape, predict_var.shape) # (400, 2, 1) (400, 0) (400, 0)
        if Y.ndim < 2:
            Y = Y.reshape(-1, 1)
        if extra is None:
            extra = nnp.nan * nnp.zeros(Y.shape)
        ...
        #print(Y.shape, predict_mean.shape, predict_var.shape, extra.shape) # (400, 2, 1) (400, 0, 1) (400, 0, 0) (400, 2, 1)
        log_density = vmap(self.likelihood.log_density, (0, 0, 0, 0, None))(
            Y.reshape(predict_mean.shape[0], -1, 1),
            predict_mean,
            predict_var,
            extra.reshape(predict_mean.shape[0], -1, 1),
            cubature
        )
...
class MarkovGaussianProcess(BaseModel):
    ...
    def __init__(self,
        ...
        if extra is None:
            extra = nnp.nan * np.zeros(Y.shape)
        self.extra = extra
